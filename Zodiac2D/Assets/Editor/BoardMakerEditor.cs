...
public class BoardMakerEditor : EditorWindow {
    ...
    private int newTileY = 0;
    TileBehavior.TileType newTileType = TileBehavior.TileType.Plains;
    TileBehavior.TileType fillTileType = TileBehavior.TileType.Plains;
    ...
    TileBehavior.TilePosition tileInformation;
    ...
    private Color plainColour = new Color(0, 255, 0); //green
    private Color roadColour = new Color(200, 200, 200); //grey
    private Color buildingColour = new Color(255, 153, 0);//yellow
    private Color baseColour = new Color(255, 51, 0);
    private Color riverColour = new Color(102, 255, 255);
    private Color seaColour = new Color(0, 0, 255);

    //textures for tiles
    private Texture2D plainsTex;
    private Texture2D roadTex;
    private Texture2D buildingTex;
    private Texture2D baseTex;
    private Texture2D riverTex;
    private Texture2D seaTex;
    ...
    private void DrawFilePanel()
    {
        ...
        //scrollPos = GUILayout.BeginScrollView(scrollPos,GUILayout.Width(position.width * sidePanelSizeRatio),GUILayout.Height(position.height));
        ...
        //button to forcefully redraw
        if(GUILayout.Button("Redraw", GUILayout.Height(buttonHeight)))
        {
            RedrawMapButtonLogic();
        }
        //object fields for textures
        plainsTex = (Texture2D)EditorGUILayout.ObjectField("Plains texture", plainsTex, typeof(Texture2D), true);
        roadTex = (Texture2D)EditorGUILayout.ObjectField("Road texture", roadTex, typeof(Texture2D), true);
        buildingTex = (Texture2D)EditorGUILayout.ObjectField("Building texture", buildingTex, typeof(Texture2D), true);
        baseTex = (Texture2D)EditorGUILayout.ObjectField("Base texture", baseTex, typeof(Texture2D), true);
        riverTex = (Texture2D)EditorGUILayout.ObjectField("River texture", riverTex, typeof(Texture2D), true);
        seaTex = (Texture2D)EditorGUILayout.ObjectField("Sea texture", seaTex, typeof(Texture2D), true);
        //end area
        GUILayout.EndArea();
    }
    ...
    //redraw map button
    private void RedrawMapButtonLogic()
    {
        //check that map exists
        if (map)
        {
    ...
    private void DrawMap()
    {
        ...
        float mapBaseY = spaceFromTop;

        //for all contents in details list
        for(int i = 0; i < boardDetails.Count; i++)
        {
            //get a rect
            Rect boardTile = new Rect(/*mapBaseX + */(boardDetails[i].position.xPos * tileWidth) + (gapBetweenTiles * boardDetails[i].position.xPos), (mapBaseY + boardDetails[i].position.yPos * tileHeight) + (gapBetweenTiles * boardDetails[i].position.yPos), tileWidth, tileHeight);
            Texture2D tex = GetTileTexture(boardDetails[i].value.type); /*new Texture2D(1, 1);*/
            //Color[] color = new Color[1];
            //color[0] = ColourTexture(boardDetails[i].value.type);
            ////tex.SetPixel(0, 0, ColourTexture(boardDetails[i].properties.type));
            //tex.SetPixels(color);
            tex.Apply();
            ...
            if(thingsHappenin.type == EventType.MouseDown && boardTile.Contains(thingsHappenin.mousePosition))
            {
                ...
                tileXText = boardDetails[i].position.xPos.ToString();
                tileYText = boardDetails[i].position.yPos.ToString();
                tileType = boardDetails[i].value.type;
            ...
            if(toX > fromX && toY > fromY)
            {
                ...
                for(int i = fromX; i <= toX; i++)
                {
                    ...
                    for(int j = fromY; j <= toY; j++)
                    {
                        ...
                        TileBehavior createdTile = newTileObject.AddComponent<TileBehavior>();
                        //assign values to new tile
                        createdTile.position.xPos = i;
                        createdTile.position.yPos = j;
                        createdTile.value.type = fillTileType;
                        //check that this tile does not already exist
                        if (!map.CheckForSameInBoard(createdTile, 0))
                        {
                            //attach this new object to the maps object as child
